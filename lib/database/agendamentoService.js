...
export async function addAgendamento(data, hora, clienteNome = null, servico = "Massagem Relaxante") {
  ...
  const list = await getFromStorage();
  const maxId = list.length ? Math.max(...list.map((a) => Number(a.id))) : 0;
  const newId = maxId + 1;
...
export async function cancelarAgendamento(id) {
  ...
  if (useSQLite()) {
    const db = getDBConnection();
    await db.runAsync("DELETE FROM agendamentos WHERE id = ?", numId);
    return;
  }
  const list = await getFromStorage();
  const novaLista = list.filter((a) => Number(a.id) !== numId);
  await setToStorage(novaLista);
}
